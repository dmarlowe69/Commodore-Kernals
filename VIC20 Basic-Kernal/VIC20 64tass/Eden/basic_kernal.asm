...
RESLST
	...
	.text	"RU",'N'+$80		; RUN
	.text	"I",'F'+$80		; IF
	.text	"RESTOR",'E'+$80	; RESTORE
	.text	"GOSU",'B'+$80		; GOSUB
	.text	"RETUR",'N'+$80		; RETURN
	.text	"RE",'M'+$80		; REM
	...
	.text	"LOA",'D'+$80		; LOAD
	.text	"SAV",'E'+$80		; SAVE
	.text	"VERIF",'Y'+$80		; VERIFY
	.text	"DE",'F'+$80		; DEF
	.text	"POK",'E'+$80		; POKE

	.text	"PRINT",'#'+$80		; PRINT#
	.text	"PRIN",'T'+$80		; PRINT
	.text	"CON",'T'+$80		; CONT
	.text	"LIS",'T'+$80		; LIST
	.text	"CL",'R'+$80		; CLR
	.text	"CM",'D'+$80		; CMD
...
LAB_E602
	...
	CMP	#$83			; compare with [RUN]
	BNE	GET2RTN			; branch if not [RUN]

					; key was [RUN] so put "LOAD",$0D,"RUN",$0D into
					; the buffer
...
LAB_EE2B
	...
	JSR	SEROUT1			; set serial DATA high
	...
	JSR	SRCLKHI			; set serial CLK high
LAB_EE38
	LDA	VIA1PA2			; get VIA 1 DRA, no handshake
...
LAB_EE40
	JSR	SRCLKLO			; set serial CLK low
	JSR	SEROUT1			; set serial DATA high
...
SRSEND
	SEI				; disable interrupts
	JSR	SEROUT1			; set serial DATA high
	JSR	SERGET			; get serial CLK status
...
LAB_EE73
	...
	JSR	SEROUT0			; else set serial DATA low
...
LAB_EE8B
	...
	LDA	VIA2PCR			; get VIA 2 PCR
	AND	#$DF			; set CB2 low, serial DATA out high
	ORA	#$02			; set CA2 high, serial CLK out low
...
LAB_EEB9
	...
	BCC	LAB_EF09		; ATN high, delay, CLK high then DATA high, branch always
...
LAB_EED3
	SEI				; disable interrupts
	JSR	SEROUT0			; set serial DATA low
	JSR	SCATN			; set serial ATN high
	JSR	SRCLKHI			; set serial CLOCK high
LAB_EEDD
	JSR	SERGET			; get serial CLK status
	BCS	LAB_EEDD		; branch if CLK high
...
; ATN high, delay, CLK high then DATA high

LAB_EF09
	JSR	SCATN			; set serial ATN high

; 1ms delay, CLK high then DATA high
...
LAB_EF0F
	...
	JSR	SRCLKHI			; set serial CLK high
	JMP	SEROUT1			; set serial DATA high and return
...
LAB_EF21
	JSR	SERGET			; get serial CLK status
	BCC	LAB_EF21		; loop while CLK low
...
LAB_EF45
	JSR	SEROUT0			; set serial DATA low
	JSR	LAB_EF0C		; 1ms delay, CLK high then DATA high
	LDA	#$40			; set EOI bit
	JSR	ORIOST			; OR into I/O status byte
...
LAB_EF58
	...
	LSR				; serial CLK into carry
	BCC	LAB_EF58		; loop while serial CLK low
...
LAB_EF66
	...
	LSR				; serial CLK into carry
	BCS	LAB_EF66		; loop while serial CLK high
...
SRCLKHI
	LDA	VIA2PCR			; get VIA 2 PCR
	AND	#$FD			; set CA2 low, serial CLK out high
	STA	VIA2PCR			; set VIA 2 PCR
	RTS
...
RSPAUSE
	PHA				; save .A
	LDA	VIA1IER			; get VIA 1 IER
	BEQ	LAB_F172		; branch if no interrupts enabled. b7 is clear on a
					; read for a MOS 6522 but set for a Rockwell 6522 so
					; the branch will never be taken for the latter
LAB_F166
	LDA	VIA1IER			; get VIA 1 IER
...
LAB_F351
	...
	LDA	#$EE			; CB2 high, RS-232 Tx
					; CB1 -ve edge, RS-232 Rx
					; CA2 high, tape motor off
					; CA1 -ve edge, [RESTORE] key
...
LAB_FA19
	...
	BNE	LAB_F9E5		;.
LAB_FA22
	JMP	RRTI			; restore registers and exit interrupt
...
INITVIA
	...
	LDA	#$DE			; CB2 low, serial DATA out high
					; CB1 +ve edge, serial SRQ in
					; CA2 high, serial CLK out low
					; CA1 -ve edge, cassette read
	STA	VIA2PCR			; set VIA 2 PCR
	...
	LDX	#$80			; OIII IIII, serial ATN out, light pen, joystick, serial data
					; in, serial CLK in
...
RSNMI
	LDA	VIA1IER			; get VIA 1 IER
	ORA	#$80			; set enable bit, b7 is clear on a read for a MOS 6522
					; and set for a Rockwell 6522
...
LAB_FF2C
	...
	BEQ	RRTI			; if no bit restore registers and exit interrupt
